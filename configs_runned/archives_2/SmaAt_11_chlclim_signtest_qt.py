...
dataloader_config={'dataset_path_inputs':path+"physics/processed_physics_1998_2023_8d_lat50_100.npy",
                   ...
                   'norm_mode':'standard',
                   'log_chl':False,
                   'anomalies':2, #0 or False or None for classic behavior, 1 for chl anomalies, 2 for physics anomalies 
                   'completion':'zeros',
...
train_config = {
    ...
    'name':'SmaAt_11_chlclim_signtest_qt',
    ...
    'comment':'SMaat 11 avec climato de Chl scaled correctement, all inputs + anomalies for ssh mld sst and sign output + loss for sign and amplitude + quantile loss'
}
...
model_config = {
    ...
    'freeze_key':'0',
    'kernels_per_layer':2,
    'chl':3, # 0 pour prédire CHL et PSC (maintient la compatibilité avec les configs passées).
...
criterion_config = {
    ...
    'Under_chl':0.,
    'KL_div':0.,
    'quantile_loss':((0.5,0.5),(0.05,0.05),(0.25,0.20),(0.75,0.20),(0.95,0.05)),
    'HingeLoss':1.,
    'details':True
}
...
scheduler_config = {
    ...
    'patience': 15,# how long to wait before updating lr
