...
class SpatioTemporalConv(nn.Module):
    ...
    def forward(self, x):
        ...
        if bn :
            x = self.activation(self.bn(self.spatial_conv(x)))
        else :
...
class DownConv(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, pooling=True, activation=nn.ReLU(), bn=True):
        ...
        self.conv = SpatioTemporalConv(self.in_channels, self.out_channels, 
                                       kernel_size=(3,3,3), stride=1, padding=1, 
                                       bias=True, bn=bn, activation=self.activation)
...
class UpConv(nn.Module):
    ...
    A ReLU/SiLU activation follows each convolution.
    """
    def __init__(self, in_channels, out_channels, activation=nn.ReLU(), bn=True):
...
class SEA(nn.Module):
    ...
    def __init__(self, channels, image_size):
        ...
        self.conv1 = SpatioTemporalConv(self.channels, self.channels, 
                                       kernel_size=(3,3,3), stride=1, padding=1, 
                                       bias=True, bn=False, activation=self.activation)
        
        self.conv2 = SpatioTemporalConv(self.channels, self.channels, 
                                       kernel_size=(3,3,3), stride=1, padding=1, 
                                       bias=True, bn=False, activation=self.activation)
...
class SPA(nn.Module):
    ...
    def __init__(self, channels, time_size):
        ...
        self.conv = SpatioTemporalConv(self.channels, self.channels, 
                                       kernel_size=self.kernel, stride=1, padding=1, 
                                       bias=True, bn=False, activation=self.activation) #c'est quoi ce fameux kernel size ? pas convaincu de la conv 3d
        self.sigmoid=nn.Sigmoid()
...
class RSSAB(nn.Module):
    """
    performs N-SSAB
    the original paper doesnt mention any number so N it's basically a parameter to tune
    """
    def __init__(self, n_ssab, channels, input_size):
        ...
        self.conv=SpatioTemporalConv(self.channels, self.channels, 
                                       kernel_size=(3,3,3), stride=1, padding=1, 
                                       bias=True, bn=False, activation=self.activation) #peut ếtre possible de mettre de la batchnorm ici
...
class ED_DRAP(nn.Module):
    ...
    def __init__(self, size_input=(10,128,128), activation='ReLU',
        ...
        self.first_conv=SpatioTemporalConv(self.channel_input, self.filters_list[0], 
                                       kernel_size=(3,3,3), stride=1, padding=1, 
                                       bias=True, bn=True, activation=self.activation)
        ...
        self.final_conv=SpatioTemporalConv(self.filters_list[0], self.channel_output, 
                                       kernel_size=(3,3,3), stride=1, padding=1, 
                                       bias=True, bn=False, activation=self.activation)
