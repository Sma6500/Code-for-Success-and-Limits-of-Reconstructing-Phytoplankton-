...
def upconv2x2(in_channels, out_channels):
    return nn.Sequential(nn.Upsample(mode='bicubic', scale_factor=2),conv1x1(in_channels, out_channels))
...
class DownConv(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, pooling=True, activation=nn.ReLU(), residual_block=False):
        ...
        if self.residual_block:
            self.conv_res=conv1x1(self.in_channels, self.out_channels)
...
class UpConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, 
                 merge_mode='concat', up_mode='transpose', activation=nn.ReLU(), residual_block=False):
    ...
    def forward(self, from_down, from_up):
        ...
        if self.merge_mode == 'concat':
            res = torch.cat((from_up, from_down), 1)
        else:
            res = from_up + from_down
        x = self.activation(self.conv1(res))
...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes, in_channels=8, depth=5,
                 ...
                 transfo_field=False, residual_block=False):
        """
        Arguments:
            in_channels: int, number of channels in the input tensor.
        ...
        for i in range(depth):
            ...
            down_conv = DownConv(ins, outs, pooling=pooling, activation=self.activation, residual_block=self.residual_block)
        ...
        for i in range(depth-1):
            ...
            up_conv = UpConv(ins, outs,
                merge_mode=merge_mode, activation=self.activation, residual_block=self.residual_block)
...
if __name__ == "__main__":
    ...
    model = UNet(1, depth=4, in_channels=10, merge_mode='concat', activation='ReLU', transfo_field=False, residual_block=False)
    model.to(device=0)
    ...
    # output_names = ['yhat']
    a=torch.Tensor(np.random.random(((1,10,360,100)))).to(device=0)
