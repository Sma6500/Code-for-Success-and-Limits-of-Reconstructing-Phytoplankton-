...
class Dataset_psc_chl(Dataset):
    ...
    def __init__(self, mask, indexs, scaler_phy, scaler_chl, dataloader_config, transform=None, completion='inf value'):
        ...
        self.physics_dataset=xr.open_dataset(self.dataloader_config['dataset_path_inputs'], chunks={'time': 100}).to_array(dim='variables').data[:,indexs].compute()
        self.chl_dataset = xr.open_dataset(self.dataloader_config['dataset_path_chl_psc'], chunks={'time': 100}).to_array(dim='variables').data[0, indexs].compute()
        self.psc_dataset = xr.open_dataset(self.dataloader_config['dataset_path_chl_psc'], chunks={'time': 100}).to_array(dim='variables').data[1:,indexs].compute()
        ...
        #self.physics_dataset = np.where(np.isinf(self.physics_dataset,), np.nan, self.physics_dataset)
        
        #self.chl_dataset, self.psc_dataset=self.process_chl_psc(self.chl_dataset, self, new_index)
        
        #chl_masked=np.where(self.mask[index]==1,chl,np.nan)
        #psc_masked=np.where(np.repeat(self.mask[np.newaxis,index],3,axis=0)==1,psc,np.nan)
    ...
    def __getitem__(self, index):
        ...
        chl = np.where(self.mask[index]==2,chl,np.nan)
        psc = np.where(np.repeat(self.mask[np.newaxis,index],3,axis=0)==2,psc,np.nan)
        
        chl_masked=np.where(self.mask[index]==1,chl,np.nan)
        psc_masked=np.where(np.repeat(self.mask[np.newaxis,index],3,axis=0)==1,psc,np.nan)

        # print(self.scaler_chl.mean)
        # print(self.scaler_phy.mean)
        # on normalise
...
def get_dataloaders(dataloader_config):
    ...
    chl = xr.open_dataset(dataloader_config['dataset_path_chl_psc'], chunks='auto').to_array(dim='variables').data[0]
    ...
    random_index=np.random.randint(0,cloud_data.shape[0],size=(chl.shape[0]))
...
########################################################################################################
    chl=da.where(random_mask>=1,chl,np.nan)
    ...
    random_mask_train, random_mask_valid, random_mask_test = random_mask[train_idx], random_mask[valid_idx], random_mask[test_idx]
    ...
    if dataloader_config['norm_mode']: 
        ...
        physics_train = scaler_phy.fit(physics_train)
        chl = scaler_chl.fit(chl,dims_first=False)
    ...
    test_generator = DataLoader(test_set, batch_size=1, shuffle=False)#,pin_memory=True,num_workers=4)
