...
test pour essayer de mieux reproduire l'interannuel, on prédit que l'interannuel calculé après la lognormalisation
...
class Dataset_psc_chl(Dataset):
    ...
    def __getitem__(self, index):
        ...
        chl_decomposed=torch.Tensor(self.dataset_chl_decomposed.T[index])
        ...
        if self.transform is not None:
            ...
            chl_decomposed=self.transform(chl_decomposed)
                
            
        return inputs, (torch.unsqueeze(chl_decomposed, dim=0))
