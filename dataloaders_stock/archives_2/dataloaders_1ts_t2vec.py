...
class Dataset_psc_chl(Dataset):
    ...
    def __init__(self, physics, chl, time, transform=None, completion='inf value'):
        ...
        self.time=np.expand_dims(time, 1)
    ...
    def __getitem__(self, index):
        ...
        inputs=np.concatenate((self.dataset_physics[index],self.time[index]))
...
def get_dataloaders(dataloader_config):
    ...
    mask=~np.isnan(physics[0,-3]) #we take bathy and create a continent mask

    sst_mean=np.concatenate([np.nanmean(physics.reshape(26,46,8,100,360),axis=0)]*26,axis=0)[:,1]
    physics[:,1]=physics[:,1]-sst_mean
    physics=np.concatenate((physics[:,:2],physics[:,3:4]),axis=1)
    #physics=np.expand_dims(physics[:,1], axis=1)#only sst
    ...
    for t in range(time.shape[0]):
        ...
        time_encoder=(t+231)/(40*46) #we start in 1993
    ...
    time_trainset=np.concatenate([time[index] for index in train_indices])
...
#             chl_masked_validset[tuple(index_to_mask[i])]=np.nan
    ...
    time_validset=np.concatenate([time[index] for index in valid_indices])
    ...
    time_testset=np.concatenate([time[index] for index in test_indices])
    ...
    training_set=Dataset_psc_chl(physics_trainset,chl_trainset,time_trainset,#psc_trainset, 
                                 transform=dataloader_config['transform'], 
                                 completion=dataloader_config['completion'])
    
    validation_set=Dataset_psc_chl(physics_validset,chl_validset,time_validset,#psc_validset, 
                                 transform=dataloader_config['transform'], 
                                 completion=dataloader_config['completion'])

    
    test_set=Dataset_psc_chl(physics_testset,chl_testset,time_testset,#psc_testset, 
                                 transform=dataloader_config['transform'], 
                                 completion=dataloader_config['completion'])
