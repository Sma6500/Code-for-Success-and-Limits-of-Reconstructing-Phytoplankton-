...
def get_dataloaders(dataloader_config):
    ...
    phy_mean=np.concatenate([np.nanmean(physics.reshape(24,46,8,100,360),axis=1)]*46,axis=0)
    physics=physics-phy_mean

    chl_mean=np.concatenate([np.nanmean(chl.reshape(24,46,100,360),axis=1)]*46,axis=0)
    chl=chl-chl_mean
