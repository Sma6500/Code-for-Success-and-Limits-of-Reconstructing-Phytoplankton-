...
class Dataset_psc_chl(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Return tuple
        if chl_anom is not None :
            return inputs, (chl.unsqueeze(0), chl_anom.unsqueeze(0)) 
        else :
            return  inputs, (chl.unsqueeze(0))
...
def get_dataloaders(dataloader_config):
    ...
    if dataloader_config.get('phy_anomalies', False):
        phy_anom = compute_anomalies(physics)[:, dataloader_config['phy_anomalies']]
        physics_anom_tot = compute_anomalies(physics_tot)[:, dataloader_config['phy_anomalies']]

    if dataloader_config.get('chl_anomalies', False):
        chl_anom = np.sign(compute_anomalies(chl)) * (np.log10(np.clip(np.abs(compute_anomalies(chl)), 1e-5, 10)) + 5)
    ...
    total_timesteps, timesteps_per_year = physics.shape[0], 12
    ...
    chl_anom_testset = np.zeros_like(chl_anom) if chl_anom is not None else None
    ...
    # Create datasets
    training_set = Dataset_psc_chl(physics_trainset, chl_trainset, chl_anom_trainset, transform=dataloader_config['transform'], completion=dataloader_config['completion'])
    validation_set = Dataset_psc_chl(physics_validset, chl_validset, chl_anom_validset, transform=dataloader_config['transform'], completion=dataloader_config['completion'])
    test_set = Dataset_psc_chl(physics_testset, chl_testset, chl_anom_testset, transform=dataloader_config['transform'], completion=dataloader_config['completion'])
...
if __name__=='__main__': 
    ...
    dataloader_config={'dataset_path_inputs':path+"physics/physics_completion/processed_1998_2023_reanalysis_100km_monthly_lat50_seaice.nc",
                       'dataset_path_inputs_test':None,#"path+physics/processed_physics_1993_2023_8d_lat50_100.npy",
                       'dataset_path_psc':None,#path+"PSC/1998_2023_psc4_8d_100km_lat50.npy",
                       ...
                       'stations_coord':{'SWAtl':(-55,-43),# stations withdrawed of the dataset for control
                                         ...
                                         'SCTR':(80,-3)},
                       'variables_physiques':[],
                       'phy_anomalies': [0] ,#list of which physical fields anomalies should be added
                       'chl_anomalies':True, 
                       } 
