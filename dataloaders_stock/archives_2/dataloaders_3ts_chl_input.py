...
Dataloader that loads 3 timestep (one before one after) of physics as inputs (regardless the number of channels) and
loads chl and psc as outputs
...
#from utils.hplc import hplc_points
hplc_points=np.load("/home/luther/Documents/npy_data/insitu/Hourany/hplc_correspondings.npy")
# +---------------------------------------------------------------------------------------+ #
# |                                                                                       | #
...
class Dataset_psc_chl(Dataset):
    ...
    def __getitem__(self, index):
        ...
        chl=np.expand_dims(self.dataset_chl[index-1:index+2],axis=1)
        #on concatène parce qu'on a de la channel attention qui servira de 
        #d'attention temporelle et parce que les conv 3d ou 2d+1 c'est trop coûteux
        
        inputs=np.concatenate([np.concatenate([inputs[i],chl[i]]) for i in range(3)])
...
def get_dataloaders(dataloader_config):
    ...
    for t,la,lo in hplc_points:
        psc[t,:,la,lo]=np.nan
        chl[t,la,lo]=np.nan
    
    if dataloader_config['log_chl']:
        epsilon=1e-5
        chl=np.clip(chl,epsilon,None)
        chl=np.log10(chl)
    ...
    train_indices = []
    valid_indices = []
    test_indices = slice(-46*2,None)
    
    # Iterate over years and append indices to sets
    for i in range(total_years-2):
...
#bon avec un break ya probablement moyen de s'en sortir mieux
    for i in range(len(index_to_mask)):
        if i/len(index_to_mask)<0.1: #as we already have year, i only mask 10% pixels
    ...
    psc_validset=np.concatenate([psc[index] for index in valid_indices]+[psc_masked_validset])
    chl_validset=np.concatenate([chl[index] for index in valid_indices]+[chl_masked_validset])            
    ...
    if dataloader_config['norm_mode']: 
        ...
        physics_validset=np.swapaxes(scaler.fit_transform(np.swapaxes(physics_validset, 1, -1)),-1,1)            
        physics_testset=np.swapaxes(scaler.fit_transform(np.swapaxes(physics_testset, 1, -1)),-1,1)            

        chl_trainset=np.squeeze(scaler.fit_transform(np.expand_dims(chl_trainset, -1)))            
        chl_validset=np.squeeze(scaler.fit_transform(np.expand_dims(chl_validset, -1)))          
        chl_testset=np.squeeze(scaler.fit_transform(np.expand_dims(chl_testset, -1)))         
