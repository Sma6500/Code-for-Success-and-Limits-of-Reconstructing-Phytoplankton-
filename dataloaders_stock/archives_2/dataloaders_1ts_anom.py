...
class Dataset_psc_chl(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return inputs, (psc, torch.unsqueeze(chl, dim=0), torch.unsqueeze(chl_anom, dim=0))
...
def get_dataloaders(dataloader_config):
    ...
    physics_tot=np.load(dataloader_config.get("dataset_path_inputs_test","/home/luther/Documents/npy_data/physics/processed_physics_1993_2023_8d_lat50_100.npy"))*mask_coast
    ...
    chl_anom_trainset=np.concatenate([chl[index] for index in train_indices])
    ...
    chl_anom_validset=np.concatenate([chl[index] for index in valid_indices])            
    ...
    training_set=Dataset_psc_chl(physics_trainset,chl_trainset,psc_trainset, chl_anom_trainset,
                                 transform=dataloader_config['transform'], 
                                 completion=dataloader_config['completion'])
    
    validation_set=Dataset_psc_chl(physics_validset,chl_validset,psc_validset, chl_anom_validset,
                                 transform=dataloader_config['transform'], 
                                 completion=dataloader_config['completion'])

    
    test_set=Dataset_psc_chl(physics_testset,chl_testset,psc_testset, chl_anom_testset,
                                 transform=dataloader_config['transform'], 
                                 completion=dataloader_config['completion'])
