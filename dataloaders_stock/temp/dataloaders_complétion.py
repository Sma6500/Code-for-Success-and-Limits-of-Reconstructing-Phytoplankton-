...
class Dataset_psc_chl(Dataset):
    ...
    def __init__(self, mask, indexs, scaler_phy, scaler_chl, dataloader_config, transform=None, completion='inf value'):
        ...
        self.chl_dataset = xr.open_dataset(self.dataloader_config['dataset_path_chl_psc'], chunks={'time': 100}).to_array(dim='variables').sel(variables='CHL').data[indexs].compute()
        self.psc_dataset = xr.open_dataset(self.dataloader_config['dataset_path_chl_psc'], chunks={'time': 100}).to_array(dim='variables').sel(variables=['Micro','Nano','Pico']).data[:,indexs].compute()
...
def get_dataloaders(dataloader_config):
    ...
    random_index=np.random.randint(0,cloud_data.shape[0],size=(chl.shape[0])) #we've got a problem here folks
...
########################################################################################################
    ...
    random_mask_train, random_mask_valid, random_mask_test = random_mask[train_idx], random_mask[valid_idx], random_mask
