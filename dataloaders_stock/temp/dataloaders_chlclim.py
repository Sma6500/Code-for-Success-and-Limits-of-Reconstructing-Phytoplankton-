...
def get_dataloaders(dataloader_config):
    ...
    chl_mean=np.concatenate([np.nanmean(chl.reshape(26,46,100,360),axis=0)]*26,axis=0)
    chl=chl-chl_mean
    #on se débrouille pour que la chloro soit positive
    chl=chl+np.abs(np.nanmin(chl,axis=0))
