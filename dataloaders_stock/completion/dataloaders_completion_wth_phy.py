...
class Dataset_psc_chl(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataloader_config['log_chl']:
            ...
            chl=np.log10(chl)
        #on met les psc dans la bonne range (pour les psc de roy)
        if np.nanmean(np.nansum(psc,axis=0))>80:
            psc=psc/100
        ...
        inputs=np.concatenate((chl_masked[np.newaxis], psc_masked))
...
if __name__=='__main__': 
    ...
    dataloader_config={'dataset_path_inputs':path+"physics/physics_completion/reduced_data.nc",
                       'dataset_path_chl_psc':path+"completion/AVW/chl_psc_avw_roy_1d_glob_100km_1998_2023.nc",
