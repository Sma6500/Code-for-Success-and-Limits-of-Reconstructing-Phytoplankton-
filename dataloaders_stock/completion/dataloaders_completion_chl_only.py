...
class Dataset_psc_chl(Dataset):
    ...
    def __init__(self, mask, indexs, scaler_phy, scaler_chl, dataloader_config, transform=None, completion='inf value', test=False):
        ...
        if not(test):
            self.physics_dataset=xr.open_dataset(self.dataloader_config['dataset_path_inputs'], chunks={'time': 100})[['mld','sst','sss','ssh','solar','sithick','norm_currents','vort_winds']].to_array(dim='variables').data[:,indexs].compute()
            ...
            self.physics_dataset=xr.open_dataset(self.dataloader_config['dataset_path_inputs'], chunks={'time': 100})[['mld','sst','sss','ssh','solar','sithick','norm_currents','vort_winds']].to_array(dim='variables').data
    ...
    def __getitem__(self, index):
        ...
        inputs=np.concatenate((physics, chl_masked[np.newaxis]))
        ...
        return inputs, (chl) #attnetion pas la bonne version ça bug ici à cause du tuple, chequer le dataloader dans result yu completion
...
def get_dataloaders(dataloader_config):
    ...
    physics = xr.open_dataset(dataloader_config['dataset_path_inputs'])[['mld','sst','sss','ssh','solar','sithick','norm_currents','vort_winds']].to_array(dim='variables').data
...
if __name__=='__main__': 
    ...
    dataloader_config={'dataset_path_inputs':path+"physics/physics_completion/processed_1998_2023_reanalysis_100km_monthly_seaice.nc",
                       'dataset_path_chl_psc':path+"completion/yu/Yu2023_notransfo_1m_1998-2023_glob_100.nc",
